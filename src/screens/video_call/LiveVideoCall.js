...
const LiveVideoCall = ({ route }) => {
  ...
  const sheetReportUserRef = React.useRef();
  ...
  const handleCut = () => {
    ...
    // CALL_RATE_INTERVAL = setInterval(checkRemainingCoin, 6000); /// one min fast forward
  };
  ...
  const sendInvitation = async () => {
    ...
    console.log(response, "api_sendVideoCallInvitation");
    if (response.isSuccess && response.statusCode === 200) {
  ...
  const handleReportPress = async (text) => {
    ...
    try {
      const payload = {
        token: auth?.accessToken,
        otherUserId: CALLING_TO_USER_ID || USER_ID_FOR_REPORT,
        userID: auth?.user?.id,
        msg: text,
      };
  ...
  const handleReportUserSheet = (bool) => {
    if (bool) {
      SheetManager.show("sheetReportUserFormVideoCall");
    } else {
  ...
  useEffect(() => {
    ...
    HISTORY_ID = null;
    HISTORY_ID = route?.params?.historyId;
    ...
    if (CALL_MODE === "RANDOM" && CALL_TYPE === "RECIVER") {
      ...
      if (CALL_MODE === "NORMAL" && CALL_TYPE === "RECIVER") {
        ...
        //caller
        console.log("CALLER MODE", route.params);
        const { api_uid, api_agora_token, channel_id } = route.params;
        if ((api_uid, api_agora_token, channel_id)) {
          setChannelId(channel_id);
          setRtcToken(api_agora_token);
    ...
    <>
      ...
      <View style={styles.max}>
        ...
        <TouchableOpacity
          ...
          onPress={() => handleReportUserSheet(true)}
        >
          {reportLoading ? (
            <ActivityIndicator size={"small"} color={"#fff"} />
          ) : (
            <Fontisto name="flag" size={25} color={"#fff"} />
          )}
        </TouchableOpacity>
        ...
        <ActionSheet id="sheetReportUserFormVideoCall" ref={sheetReportUserRef}>
          <View
            style={{
              height: 340,
            }}
